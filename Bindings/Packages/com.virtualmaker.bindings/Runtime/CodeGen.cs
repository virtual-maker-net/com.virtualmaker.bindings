...
namespace VirtualMaker.Bindings
{
    internal static class BindingsCodeGen
    {
        ...
        private static string GeneratePropertyInfo(Type componentType, string prop, string name)
        {
            ...
            s += tab + $"public static void Bind{prettyProp}{generic}(this {name} obj, UnityEngine.Object context, IProperty<{propType}> property)" + Environment.NewLine;
            s += tab +  $"    => Bind(context, property, v => obj.{prop} = v{suffix});" + Environment.NewLine;

            s += tab + $"public static void Bind{prettyProp}{generic}(this {name} obj, IProperty<{propType}> property)" + Environment.NewLine;
            s += tab + $"    => Bind{prettyProp}(obj, obj, property);" + Environment.NewLine;

            s += tab + $"public static void Bind{prettyProp}{generic}(this {name} obj, UnityEngine.Object context, Func<{propType}> func)" + Environment.NewLine;
            s += tab +  $"    => BindUpdate(context, () => obj.{prop} = func(){suffix});" + Environment.NewLine;
